// Copyright (c) Ame (Akeoot/Akeoott) <akeoot@pm.me>. Licensed under the LGPL-3.0 Licence.
// See the LICENSE file in the repository root for full license text.

using ZenMonitor.Core.Abstractions.Telemetry;
using ZenMonitor.Core.Models.Telemetry;

namespace ZenMonitor.Core.Services;

/// <summary>
/// No-op <see cref="ICpu"/> implementation that returns all-zero / empty defaults.
/// Used as a fallback when the platform is unsupported or detection fails.
/// </summary>
public sealed class NullCpu : ICpu
{
    /// <summary>No-op update — does nothing.</summary>
    public void Update() { }

    /// <summary>Returns an empty string.</summary>
    public string GetCpuName() => "";

    /// <summary>Returns 0.</summary>
    public double GetCpuSpeed() => 0;

    /// <summary>Returns 0.</summary>
    public int GetCpuUsage() => 0;

    /// <summary>Returns 0.</summary>
    public int GetCpuTemp() => 0;

    /// <summary>Returns 0.</summary>
    public double GetPowerDraw() => 0;

    /// <summary>Returns an empty array.</summary>
    public CpuCoreSpeed[] GetCoreSpeeds() => [];

    /// <summary>Returns an empty array.</summary>
    public CpuCoreUsage[] GetCoreUsages() => [];

    /// <summary>Returns an empty array.</summary>
    public CpuCoreTemp[] GetCoreTemps() => [];
}
